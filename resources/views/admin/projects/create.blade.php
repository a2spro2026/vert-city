@extends('layouts.admin')

@section('title', 'Nouveau projet')
@section('page-title', 'Nouveau projet')

@section('content')
    <div class="mb-7">
        <a href="{{ route('admin.projets.index') }}" class="text-sm font-medium text-slate-400 hover:text-[#376127]">← Retour aux projets</a>
        <h2 class="mt-3 text-2xl font-semibold tracking-tight">Créer un projet</h2>
        <p class="mt-2 text-slate-500">Renseignez les informations principales du projet immobilier.</p>
    </div>

    <form method="POST" action="{{ route('admin.projets.store') }}" enctype="multipart/form-data">
        @csrf
        @include('admin.projects._form', ['submitLabel' => 'Créer'])
    </form>
@endsection
